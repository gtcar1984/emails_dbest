...
if POS < MAX:
    ...
    # Carrega o caminho na posição atual
    ...
    for dest in DATA_LIST:
        try:
            ...
            # Assunto do email
            # Carrega o assunto na posição atual
            subject = info.get('SUBJECTS')[POS]
            ...
            with smtplib.SMTP(smtp_server, int(smtp_port)) as server:
                ...
                with open(PATH_LOG, 'a+', encoding='utf-8') as log:
                    log.write((f'{datetime.strftime(datetime.now(), "%d/%m/%Y - %H:%M:%S")} - Email para {
                              dest["NOME"]} da empresa {dest["EMPRESA"]} enviado com sucesso!\n'))

        except smtplib.SMTPRecipientsRefused:
            with open(PATH_LOG, 'a', encoding='utf-8') as log:
                log.write((f'{datetime.strftime(
                    datetime.now(), "%d/%m/%Y - %H:%M:%S")} - Email para {dest["NOME"]} FALHOU!\n'))
    ...
    with open(PATH_LOG, 'a', encoding='utf-8') as log:
        log.write((f'{datetime.strftime(datetime.now(),
                  "%d/%m/%Y - %H:%M:%S")} - Sequência chegou ao fim!\n'))

# Apenas para checar o tempo total de execução
with open(PATH_LOG, 'a', encoding='utf-8') as log:
    log.write(f'O tempo total de execução do main foi {
